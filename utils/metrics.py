...
class micro:
    ...
    def get_trip(self, ner_pred, re_head_pred, re_tail_pred):
        ...
        ner_pred = torch.where(
            ner_pred > 0, torch.ones_like(ner_pred), torch.zeros_like(ner_pred)
        )
        ...
        for r in range(relation):
            ...
            for i in range(seq_len * seq_len):
                if i in re_head_pred_index:
                    ...
                    if (
                        subj_head not in ner_map.keys()
                        or obj_head not in ner_map.keys()
                    ):
                        continue
    ...
    def count_num(
        ...
        ner_pred = torch.where(
            ner_pred >= 0.5, torch.ones_like(ner_pred), torch.zeros_like(ner_pred)
        )
        re_pred_head = torch.where(
            re_pred_head >= 0.5,
            torch.ones_like(re_pred_head),
            torch.zeros_like(re_pred_head),
        )
        re_pred_tail = torch.where(
            re_pred_tail >= 0.5,
            torch.ones_like(re_pred_tail),
        ...
        for i in range(batch):
            ...
            re_label_set = self.get_trip(
                ner_label_batch, re_label_head_batch, re_label_tail_batch
            )

            re_pred_head_batch = re_pred_head[:, :, i, :]
            re_pred_tail_batch = re_pred_tail[:, :, i, :]
            re_pred_set = self.get_trip(
                ner_pred_batch, re_pred_head_batch, re_pred_tail_batch
            )
            ...
            for trip in re_right:
                ...
                object = trip[2]
                if (
                    ner_right_batch[subject[0], subject[1]] > 0
                    and ner_right_batch[object[0], object[1]] > 0
                ):
                    right_num += 1
    ...
    def count_num(
        ...
        for r in range(len(self.rel2idx)):
            ...
            for i in range(batch):
                ...
                re_label_set = self.get_trip(
                    ner_label_batch, re_label_head_batch, re_label_tail_batch, r
                )

                re_pred_head_batch = re_pred_head[:, :, i, r]
                re_pred_tail_batch = re_pred_tail[:, :, i, r]
                re_pred_set = self.get_trip(
                    ner_pred_batch, re_pred_head_batch, re_pred_tail_batch, r
                )
