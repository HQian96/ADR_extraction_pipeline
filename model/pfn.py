...
class LinearDropConnect(nn.Linear):
    ...
    def sample_mask(self):
        if self.dropout == 0.0:
            ...
            mask = self.weight.new_empty(self.weight.size(), dtype=torch.bool)
...
class pfn_unit(nn.Module):
    def __init__(self, args, input_size):
        ...
        self.hidden_transform = LinearDropConnect(
            args.hidden_size, 5 * args.hidden_size, bias=True, dropout=args.dropconnect
        )
...
class encoder(nn.Module):
    ...
    def hidden_init(self, batch_size):
        h0 = (
            torch.zeros(batch_size, self.args.hidden_size)
